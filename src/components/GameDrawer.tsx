...
interface GameDrawerProps {
  ...
  onCreate: (title: string, importFile: string) => Promise<void>
  onUpdateMetadata: (id: string, title: string, note: string) => void
...
export default function GameDrawer(props: GameDrawerProps) {
  ...
  function saveEditing() {
    if (!editingId) return
    props.onUpdateMetadata(editingId, draftTitle, draftNote)
    setEditingId('')
  }
  ...
  async function confirmCreate() {
    ...
    setActionError('')
    try {
      await props.onCreate(createTitle, importFile)
      setCreateOpen(false)
    } catch (error) {
      setActionError(toMessage(error))
    } finally {
          ...
          {props.games.map((game) => editingId === game.id ? (
            ...
            <div className={`game-list-item ${game.id === props.activeGameId ? 'active' : ''}`} key={game.id}>
              ...
              <div className="game-list-actions">
                ...
                <button onClick={() => { setExportGame(game); setActionError('') }} title="导出 RPGBox 文件" disabled={working}><Download size={15} /><span>导出</span></button>
