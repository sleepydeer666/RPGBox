...
export interface GameDrawerProps {
  ...
  onCreate: (title: string, importSource: RpgboxImportSource | null, nsfwEnabled: boolean, onPortraitProgress?: (completed: number, total: number) => void) => Promise<void>
...
export default function GameDrawer(props: GameDrawerProps) {
  ...
  async function confirmCreate() {
    ...
    try {
      await props.onCreate(createTitle, importSource, createNsfwEnabled, (completed, total) => setImportProgress({ completed, total }))
      setCreateOpen(false)
    } catch (error) {
