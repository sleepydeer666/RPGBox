        ...
        <div className="game-settings-content">
          {tab === 'ai' && <section className="game-tab-panel ai-settings-panel">
            ...
            <div className="form-section"><h3>模型参数</h3>
              ...
              <ParameterSlider label="温度（0为不设置）" valueLabel={game.aiSettings.temperature === 0 ? '不设置' : undefined} min={0} max={2} step={0.05} value={game.aiSettings.temperature} onChange={(temperature) => patchGame({ aiSettings: { ...game.aiSettings, temperature } })} />
              <ParameterSlider label="Top P（0为不设置）" valueLabel={game.aiSettings.topP === 0 ? '不设置' : undefined} min={0} max={1} step={0.05} value={game.aiSettings.topP} onChange={(topP) => patchGame({ aiSettings: { ...game.aiSettings, topP } })} />
          ...
          {tab === 'rules' && <section className="game-tab-panel">
            ...
            <div className="form-section"><h3>叙事模式切换规则</h3><p className="form-section-description">描述不同叙事模式之间的切换条件，以及不同章节或剧情内容应采用的叙事模式。留空时不会加入系统提示词。</p><DeferredTextarea className="game-prompt-textarea" value={game.narrativeModeRulesPrompt ?? ''} onCommit={(narrativeModeRulesPrompt) => patchGame({ narrativeModeRulesPrompt })} placeholder="例如：日常章节使用正常模式；进入亲密情节前通过选项切换至 NSFW 模式……" /></div>
